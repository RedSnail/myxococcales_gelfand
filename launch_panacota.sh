...
do
	...
	# mv ${fam}_pan/LSTINFO*.txt ${fam}_pan/LSTINFO-$fam.txt
	# PanACoTA annotate  --info ${fam}_pan/LSTINFO-$fam.txt -r ${fam}_pan --threads 4 -n ${fam:0:4}

	for ac in ${acs[@]}
	do
		cat ~/myxococcales_gelfand/Bulk_pan/LSTINFO-LSTINFO-Bulk.lst | grep $ac
	done | \
	       gawk -F'\t' -v name=${fam:0:4} 'BEGIN {i = 1} {print name ".0921." sprintf("%05d", i) "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" $6; i += 1}' | \
	       (echo "$(head -n 1 ~/myxococcales_gelfand/Bulk_pan/LSTINFO-LSTINFO-Bulk.lst)" && cat) > ~/myxococcales_gelfand/${fam}_pan/LSTINFO-LSTINFO-${fam:0:4}.lst
	...
	while read old new
	do
		cp Bulk_pan/Proteins/$old.prt ${fam}_pan/Proteins/$new.prt
		sed -i "s|${old}|${new}|g" ${fam}_pan/Proteins/$new.prt
		cp Bulk_pan/Genes/$old.gen ${fam}_pan/Genes/$new.gen
		sed -i "s|${old}|${new}|g" ${fam}_pan/Genes/$new.gen
	done < ${fam}_pan/sname_mapping.tsv
	PanACoTA pangenome -l ~/myxococcales_gelfand/${fam}_pan/LSTINFO-LSTINFO-${fam:0:4}.lst -n ${fam:0:4} -d ~/myxococcales_gelfand/${fam}_pan/Proteins/ -o ~/myxococcales_gelfand/${fam}_pan/ --threads 8 -i 0.6
